...
`, version))
	...
	Context("when etcd is a version alias", func() {
		JustBeforeEach(func() {
			etcdURL = "director-latest"
		...
		It("returns a release stub", func() {
			...
			Expect(etcdRelease.URL).To(Equal(""))
			Expect(etcdRelease.Version).To(Equal("latest"))
		})

	})

	Context("when etcd is a path to something", func() {
